...
class GNNEstimator:
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # Select only nodes that have labels
            y = self.data.y.to(self.device).float()
            mask = torch.arange(y.size(0), device=self.device)  # first len(y) nodes
            loss = F.binary_cross_entropy_with_logits(out.squeeze()[mask], y)
